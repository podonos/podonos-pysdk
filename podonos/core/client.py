...
class Client:
    ...
    def get_evaluation_list(self) -> List[Dict[str, Any]]:
        ...
        Each row carries `progress` (0 to 100), `started_time` and `ended_time`
        alongside the evaluation's identity, so a completed evaluation can be
        ...
        All three fields can be None. `started_time` and `ended_time` are None until the
        evaluation starts and ends; `progress` is None on a backend that does not report it
        yet, so guard before comparing.
